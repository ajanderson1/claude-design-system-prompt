...
description: Use before any hi-fi design work when no brand or design system exists to match. Proposes four distinct visual directions (typography, color, density, mood, component style) and commits to one, so the design has an intentional look instead of a default AI aesthetic.
---
